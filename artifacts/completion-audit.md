...
The implementation is versioned `1.2.1`. Development and verification used only isolated fixtures outside reviewed user repositories.
...
| Versioned, hash-valid evidence | `src/types.ts`, `src/core/schema.ts`, `schema/scan-result.schema.json`; stable content-addressed evidence/finding/scan IDs, exact source hashes/ranges, and explicit complete/partial/abstained outcomes | foundation, completeness, TypeScript range, Python UTF-16, and export tests |
| Conservative provider/policy separation | `src/providers/provider.ts`, `src/providers/federation.ts`, `src/policy/engine.ts`, `library/rules.yaml`; provider count never raises authority | policy, provider degradation, provider-count, hard-negative, and malformed-input tests |
...
| Guarded patch laboratory | separate baseline/candidate Git worktrees, exact configured argv, Bubblewrap without a host-root mount, separate network/PID/IPC/UTS namespaces, scrubbed environment, private HOME/tmp, source hash guards, rollback | lab tests cover host-secret isolation, argument extension rejection, stale hashes, explicit application, rollback, deletion, R3, critical paths, and trust |
| Test amplification and bounded experiments | property/metamorphic/shadow checks, mutation score, regression generation, invariants, advisory equality saturation, CEGIS, local retrieval, evidence-citing critics | experiment tests cover exhausted-domain verification, sampled-domain inconclusiveness, counterexamples, unsupported syntax, citations, and repository-local retrieval |
| Dynamic/formal adapters | gated SMT expression-equivalence and Alive2-compatible LLVM translation validation with declared assumptions/timeouts and abstention | adapter tests require exact configured isolated commands, reject suffix extension, and verify bounded verdict parsing |
...
- Only an exhausted declared finite domain can verify a bounded expression experiment; sampling and equality saturation remain advisory.
...
- The package is published under Apache-2.0 through GitHub, npm, and the Pi package gallery; publication status is verified separately from code-quality claims.
