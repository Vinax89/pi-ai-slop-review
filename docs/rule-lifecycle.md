...
4. **Verify:** add semantic/provider evidence, exact ranges/hashes, hard negatives, and repository-separated cases.
5. **Calibrate:** measure precision, abstention, reviewer outcomes, Wilson bounds, and unsafe actions.
6. **Propose:** permit a patch proposal only when the executable `rules.yaml` cap, evidence score, risk policy, counterevidence, and local health all permit it.
